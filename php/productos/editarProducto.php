<?php
    ...
    $errorNombre = $errorDescripcion = $errorPrecio = $errorCategoria = $errorImagen = $success = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($errorNombre) && empty($errorDescripcion) && empty($errorPrecio) && empty($errorCategoria) && empty($errorImagen)) {
            $sql = "UPDATE producto SET nombre=?, descripcion=?, precio=?, categoria=?, imagen=? WHERE id_producto=?";
            $stmt = $_conexion->prepare($sql);
            $stmt->bind_param("ssdssi", $nombre, $descripcion, $precio, $categoria, $imagenFinal, $idProducto);
            if ($stmt->execute()) {
                ...
                $errorImagen = "Error al actualizar el producto: " . $_conexion->error;
            }
        }
    }
?>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <?php if (!empty($producto['imagen'])): ?>
                        <p class="text-sm mt-2">Imagen actual: <a href="<?= htmlspecialchars($producto["imagen"]) ?>" target="_blank" class="text-blue-500 underline">Ver imagen</a></p>
                    <?php endif; ?>
                ...
                <div class="mb-4">
                    ...
                    <select required id="categoria" name="categoria" class="form-select form-select-lg">
                        ...
                        <?php foreach ($categorias as $cat): ?>
                            <option value="<?= htmlspecialchars($cat) ?>" <?= $producto["categoria"] == $cat ? "selected" : "" ?>><?= htmlspecialchars($cat) ?></option>
                        <?php endforeach; ?>
                    </select>
