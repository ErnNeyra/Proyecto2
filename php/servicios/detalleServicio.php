...
<head>
    ...
    <link rel="stylesheet" href="../../css/nuevo.css">  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" integrity="sha512-9usAa10IRO0HhonpyAIVpjrylPvoDwiPUiKdWk5t3PyolY1cOd4DSE0Ga+ri4AuTroPR5aQvXU9xC6qOPnzFeg==" crossorigin="anonymous" referrerpolicy="no-referrer" />
...
<body class="bg-gray-100 font-sans min-h-screen flex flex-col">
        ...
        <div class="container mx-auto py-4 px-6 flex items-center justify-between">
          ...
          <nav class="flex items-center space-x-4">
                ...
                <?php
                    ...
                    $sqlServicio = "SELECT * FROM servicio WHERE id_servicio = ?";
                    $stmtServicio = $_conexion->prepare($sqlServicio);

                    if ($stmtServicio === false) {
                         die("Error preparando la consulta del servicio: " . $_conexion->error);
                    }
                    ...
                    $precio = htmlspecialchars($servicio["precio"]);
                    $categoria = htmlspecialchars($servicio["categoria"]); // Obtenemos la categoría
                    $descripcion = htmlspecialchars($servicio["descripcion"]);
                    $imagen = htmlspecialchars($servicio["imagen"]);
                    ...
                    $stmtComentarios->bind_param("i", $idServicio);
                    ...
                    $stmtPromedio->bind_param("i", $idServicio);
                    ...
                    $stmtRelacionados->bind_param("si", $categoria, $idServicio);
                    ...
                    <?php
                        ...
                        if (isset($_SESSION["usuario"]["usuario"]) && $_SESSION["usuario"]["usuario"] === $usuarioPropietario) {
                            ...
                            echo '</div>';
                             ...
                             echo '    Este servicio no te pertenece.';
                             echo '</div>';
                        }
                     ?>
    ...
    <?php if (isset($_SESSION["usuario"]["usuario"])): ?>
        <form id="formulario-comentario" method="POST" data-submit-url="guardar_comentario_servicio_ajax.php">
    ...
    <?php endif; ?>
            ...
            <div class="mt-12">
                <h2 class="text-2xl font-semibold text-gray-800 mb-4">Servicios relacionados</h2>
                 <?php if (!empty($serviciosRelacionados)): ?>
                    <div id="carrusel-relacionados-container" class="overflow-x-auto whitespace-nowrap scroll-smooth py-4 -ml-4 pl-4 relative">
                        <div id="carrusel-relacionados" class="whitespace-nowrap transition-transform duration-300">
                            <?php foreach ($serviciosRelacionados as $servicioRelacionado): ?>
                                <div class="inline-block mr-4 w-72 shadow-md rounded-md overflow-hidden border border-gray-200">
                                     <a href="detalleServicio.php?id_servicio=<?php echo htmlspecialchars($servicioRelacionado['id_servicio']); ?>" class="block">
                                        <img src="../util/<?php echo htmlspecialchars($servicioRelacionado['imagen']); ?>" alt="<?php echo htmlspecialchars($servicioRelacionado['nombre']); ?>" class="w-full h-40 object-cover">
                                    </a>
                                    <div class="p-4">
                 ...
                 <?php else: ?>
                     <p class="text-center text-gray-600">No hay servicios relacionados en esta categoría por el momento.</p>
                 <?php endif; ?>
            </div>
        </div>
    </main>
